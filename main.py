...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        # idxs_users = np.random.choice(range(stu_i), m, replace=False) # 随机顺序，一般操作
        idxs_users = np.array(range(school_n)) # 按用户顺序，因为fedavg是按顺序的
        #idxs_users = np.array(range(2))

        a =1

        for idx in idxs_users:  #  每个客户端依次训练
